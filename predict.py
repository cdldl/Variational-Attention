...
def train_model(config):
    ...
    if config['experiment'] == 'qgen':
        ...
        test_data = np.fromfile(config['data_dir'] + 'test.csv', dtype=np.float64)
        test_data = test_data.reshape((6607, 2001))
        true_test = test_data[:,-1]
        print("loaded")
        #train_data = pd.read_csv(config['data_dir'] + 'train3.csv').fillna(0)
        #val_data = pd.read_csv(config['data_dir'] + 'val3.csv').fillna(0)
        #test_data = pd.read_csv(config['data_dir'] + 'test.csv')
        #true_val = val_data['forret']
    ...
    x_train, y_train, x_test, y_test =  train_data[:,:-1],scaler.transform(np.array(train_data[:,-1]).reshape(-1, 1)), \
                                      test_data[:,:-1], scaler.transform(np.array(test_data[:,-1]).reshape(-1, 1))
    ...
    # x_test = pd.read_csv(config['data_dir'] + 'X_train.csv',skiprows=150000,nrows=50000).fillna(0).iloc[:,1:]
    # x_test = x_test.replace([np.inf, -np.inf], 0.)
    # x_test = scaler.transform(np.array(x_test))
    # y_test = pd.read_csv(config['data_dir'] + 'y_train.csv',skiprows=150000,nrows=50000).fillna(0).iloc[:,1:]
    # true_test = np.array(y_test.replace([np.inf, -np.inf], 0.)).reshape(-1, 1) #np.where(y_val>0,1,0)
    # true_test[true_test > 0.1] = 0.1
    # true_test[true_test < -0.1] = -0.1
    # y_test = scaler2.transform(np.array(true_test))
    #y_val =np.where(np.array(y_val.replace([np.inf, -np.inf], 0.)).reshape(-1, 1)>0,1,0)
    # x_train, y_train, x_val, y_val =  train_data.iloc[:,1:-1],scaler.transform(np.array(train_data['forret']).reshape(-1, 1)), \
    #                                   val_data.iloc[:,1:-1], scaler.transform(np.array(val_data['forret']).reshape(-1, 1))#, test_data[:,-1], test_data['forret']
