...
def dynamic_decode(decoder,
    ...
    with variable_scope.variable_scope(scope, "decoder") as varscope:
        ...
        print('decoder.output_dtype', decoder.output_dtype)
        ...
        print('decoder.output_size',decoder.output_size)
        print('decoder.output_dtype',decoder.output_dtype)
        import numpy as np
        # initial_outputs_ta = nest.map_structure(_create_ta, np.reshape(decoder.output_size,(-1,1)),
        #                                         decoder.output_dtype)
        initial_outputs_ta = nest.map_structure(_create_ta, decoder.output_size,
                                                decoder.output_dtype)

        print('initial_outputs_ta',initial_outputs_ta)
        ...
        def body(time, outputs_ta, state, inputs, finished, sequence_lengths, c_kl_loss):
            ...
            if impute_finished:
                ...
                print('zero_outputs',zero_outputs)
                print('next_outputs',next_outputs)
                emit = nest.map_structure( #FIXME
                    lambda out, zero: array_ops.where(finished, zero,out),
                    #lambda out, zero: array_ops.where(finished, tf.cast(tf.reshape(zero,shape=[-1]),tf.float32), tf.cast(tf.reshape(out,shape=[-1]),tf.float32)),
                    next_outputs,
                    zero_outputs)
            ...
            outputs_ta = nest.map_structure(lambda ta, out: ta.write(time, out),
                                           outputs_ta, emit)
            # outputs_ta = nest.map_structure(lambda ta, out: ta.write(time, out),
            #                                outputs_ta, tf.reshape(emit,shape=[-1]))
